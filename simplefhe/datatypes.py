# https://github.com/wgxli/simple-fhe
...
class EncryptedValue:
    ...
    def __add__(self, other):
        return self._binop(
            other, simplefhe._evaluator.add, simplefhe._evaluator.add_plain
        )

    def __sub__(self, other):
        return self._binop(
            other, simplefhe._evaluator.sub, simplefhe._evaluator.sub_plain
        )
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, int) or isinstance(other, float):
            ...
            raise NotImplementedError(
                "Only division by an unencrypted value is implemented!"
            )
    ...
    def __pow__(self, other):
        if isinstance(other, int) and other >= 0:
            ...
            if self._is_float:
                ...
                raise TypeError(
                    "Only non-negative, unencrypted integer exponents are supported in integer mode!"
                )
...
def load_encrypted_str(string: str) -> EncryptedValue:
    """Loads a saved encrypted value from the given string."""
    with tempfile.NamedTemporaryFile(delete=False) as temp_file:
        filepath = temp_file.name
    binary_string = base64.b64decode(string.encode("utf-8"))
